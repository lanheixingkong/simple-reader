...
class NavigationReader {
  ...
  static Future<EpubNavigation> readNavigation(
    ...
    if (package.Version == EpubVersion.Epub2) {
      ...
      // Get relative TOC directory path from archive root.
      // Some EPUBs keep nav.xhtml at root, e.g. "nav.xhtml". In that case
      // removeAt(0) would throw RangeError; keep it as empty prefix instead.
      final tocPathParts = _tocFileEntryPath!.split('/')..removeLast();
      if (tocPathParts.isNotEmpty) {
        tocPathParts.removeAt(0);
      }
      _tocFileEntryPath = tocPathParts.isEmpty ? '' : '${tocPathParts.join('/')}/';
